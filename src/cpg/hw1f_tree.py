...
def price_bermudan_hw1f(
    ...
    def _exercise_payoff(step, j):
        ...
        Utilise le par rate (PV des cashflows) au lieu du ZC forward.
        """
        ...
        # Compute par funding rate using HW1F bond prices
        if cpg_type == "COUPON" and freq_per_year > 0:
            ...
            for k in range(1, n_periods + 1):
                ...
                annuity += _bond_price(t_ex, T_k, r_node) * period_dt

            T_mat = t_ex + residual_years
            df_mat = _bond_price(t_ex, T_mat, r_node)

            if annuity > 1e-12:
                par_rate = (1.0 - df_mat) / annuity * 100.0
            else:
                par_rate = r_node * 100.0
        else:
            # LINEAR ACCRUAL: use zero rate
            T_mat = t_ex + residual_years
            df_mat = _bond_price(t_ex, T_mat, r_node)
            if df_mat > 1e-15 and residual_years > 0:
                par_rate = -np.log(df_mat) / residual_years * 100.0
            else:
                par_rate = r_node * 100.0

        # Add market spread
        spread_mkt = spread_market.spread_at(residual_days)
        funding_par = par_rate + spread_mkt

        moneyness = funding_par - strike
        if moneyness <= 0:
            return 0.0

        # Payoff = moneyness × notional × annuity factor
        payoff = moneyness / 100.0 * notional * residual_years
        return payoff

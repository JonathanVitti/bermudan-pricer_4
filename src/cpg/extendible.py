...
Strike (fixé à l'émission, ne bouge jamais) :
    K_i = taux_client - CS_deterministe(τ_i)
    où τ_i = maturité résiduelle à la date d'exercice i
    et CS_deterministe = courbe de spread initiale (terme-dépendante)
...
def compute_exercise_strikes(
    ...
    where τ_i = residual maturity at exercise date i (from exercise to final).
    The spread is term-dependent (not flat) and fixed at emission.

    Parameters
    ----------
    cpg : ExtendibleCPG
    spread_initial : SpreadTermStructure
        The spread curve as observed at emission (frozen).

    Returns
    -------
    Dict[datetime, float]
        Strike (in %) at each exercise date.
    """
    strikes = {}
    for ex_date in cpg.exercise_dates:
        residual_days = _days_between(ex_date, cpg.final_maturity)
        spread_pct = spread_initial.spread_at(residual_days)
        strike = cpg.client_rate - spread_pct
        strikes[ex_date] = round(strike, 6)

        log.debug(
            f"  Strike {ex_date.strftime('%Y-%m-%d')}: "
            f"client={cpg.client_rate:.2f}% - spread({residual_days}d)={spread_pct:.4f}% "
            f"= {strike:.4f}%"
        )
...
def price_extendible_cpg(
    ...
    Architecture:
        ...
        3. At each exercise node in the tree:
           ...
           - Compare funding forward vs strike K_i = client - CS_initial(τ)

...
def validate_deal(data):
    ...
    if "emission" in clean and "initial_maturity" in clean and "final_maturity" in clean:
        ...
        if fm < im:
            errors.append("Échéance finale doit être >= l'initiale.")
...
# Compute cache-buster hash at startup
...
def _file_hash(path, n=8):
    try:
        ...
        return "0"
_CSS_HASH = _file_hash("static/style.css")
_JS_HASH = _file_hash("static/app.js")
...
def cpg_index():
    ...
    body = CPG_BODY.replace(
        'href="/static/style.css"', f'href="/static/style.css?v={_CSS_HASH}"'
    ).replace(
        'src="/static/app.js"', f'src="/static/app.js?v={_JS_HASH}"'
    )
    return (
...
def cpg_upload_ext_trades():
    ...
    try:
        ...
        # Normalize column names — handles accents, spaces, French/English
        import unicodedata
        col_map = {}
        echeance_cols = []  # track ambiguous "echeance" columns
        for c in raw.columns:
            # Full accent stripping
            cl = str(c).strip()
            cl = unicodedata.normalize('NFKD', cl).encode('ascii', 'ignore').decode()
            cl = cl.lower().replace(" ", "").replace("_", "").replace("-", "")
            ...
            if "codetrans" in cl or cl == "type":
                ...
                col_map[c] = "Montant"
            elif cl == "coupon" or "taux" in cl:
                col_map[c] = "Coupon"
            elif "marge" in cl or "margin" in cl or "spread" in cl:
                col_map[c] = "Marge"
            elif "emission" in cl or "issue" in cl:
                col_map[c] = "DateEmission"
            elif "echeanceinit" in cl or ("echeance" in cl and "init" in cl) or "firstmat" in cl:
                col_map[c] = "DateEcheanceInitial"
            elif "echeancefinal" in cl or ("echeance" in cl and "final" in cl):
                col_map[c] = "DateEcheanceFinal"
            elif "echeance" in cl or "maturity" in cl or "maturite" in cl:
                echeance_cols.append(c)  # ambiguous — resolve later
            elif "freq" in cl:
                col_map[c] = "Frequence"
            elif "cusip" in cl or "isin" in cl:
                col_map[c] = "CUSIP"
            elif "rate" in cl and "Coupon" not in col_map.values():
                col_map[c] = "Coupon"
        
        # Resolve ambiguous echeance columns by position
        if echeance_cols and "DateEcheanceInitial" not in col_map.values():
            if len(echeance_cols) >= 2:
                col_map[echeance_cols[0]] = "DateEcheanceInitial"
                col_map[echeance_cols[1]] = "DateEcheanceFinal"
            elif len(echeance_cols) == 1:
                col_map[echeance_cols[0]] = "DateEcheanceFinal"  # final is the essential one
        ...
        # Helper: parse French number format (1 000,00 $ → 1000.00)
        def _parse_num(val):
            s = str(val).replace("$", "").replace("%", "").replace("\xa0", "").strip()
            # French: "1 000,50" → remove spaces, replace comma with dot
            s = s.replace(" ", "")
        ...
        # Helper: parse date (handles Timestamp, datetime, string)
        def _parse_date(val):
            s = str(val).strip().split()[0]  # drop time part
            if s == "NaT" or s == "nan" or not s:
                return ""
            return s

        for _, row in raw.iterrows():
            code = str(row.get("CodeTransaction", "COUPON")).upper().strip()
            if "LINEAR" in code or "ACCRU" in code:
                code = "LINEAR ACCRUAL"
            elif "COUPON" in code:
                code = "COUPON"
            ...
            emission = _parse_date(row["DateEmission"])
            init_mat = _parse_date(row.get("DateEcheanceInitial", row["DateEcheanceFinal"]))
            final_mat = _parse_date(row["DateEcheanceFinal"])

            montant = _parse_num(row["Montant"])
            coupon = _parse_num(row["Coupon"])
            # Fix: Excel stores 5% as 0.05. Detect and convert.
            if 0 < abs(coupon) < 0.25 and "%" not in str(row["Coupon"]):
                coupon = coupon * 100.0
            marge = _parse_num(row.get("Marge", 0)) if pd.notna(row.get("Marge")) else 0
            if 0 < abs(marge) < 0.25 and "%" not in str(row.get("Marge", "")):
                marge = marge * 100.0

            # CUSIP / FundServ
            cusip = str(row.get("CUSIP", "")).strip() if pd.notna(row.get("CUSIP")) else ""
            fundserv = str(row.get("FundServ", "")).strip() if pd.notna(row.get("FundServ")) else ""
...
def cpg_download_ext_template():
    ...
    import pandas as pd
    cols = ["CodeTransaction", "Inventaire", "Contrepartie",
            "DateÉmission", "DateEcheanceInitial", "DateEcheanceFinal",
            "Montant", "Coupon", "Marge", "Frequence", "BaseCalcul",
            "Devise", "CUSIP", "FundServ"]
    sample = [
        ["COUPON", "PRORO", 418414, "2025-12-19", "2026-12-19", "2035-12-19",
         1000, "5,00%", "0,00%", "Annuel", "ACT/365", "CAD", "CA31430XKX9", "DSN12345"],
        ["LINEAR ACCRUAL", "PRORO", 418414, "2025-12-19", "2040-12-19", "2040-12-19",
         1000, "6,00%", "0,00%", "Maturité", "ACT/365", "CAD", "CA31393GQ0", "DSN12345"],
    ]
    df = pd.DataFrame(sample, columns=cols)
    tmp = os.path.join(tempfile.gettempdir(), "ext_template.xlsx")
    df.to_excel(tmp, index=False, engine="openpyxl")
    return send_file(tmp, as_attachment=True, download_name="modele_epargne_terme.xlsx")
    ...
    try:
        ...
        result = {
            ...
            "calibration": {
                ...
                "note": "Strike = taux client. Sous-jacent = par rate OIS + spread marché. Backward induction HW1F (arbre trinomial) pour la time value. Payoff = notional − VA(cashflows client).",
            },
            "vol_monitor": vm,

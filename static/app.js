...
function runAllPricing(){
  ...
  /* Show status in step3 immediately */
  function setStatus(msg,isErr){
    var el=document.getElementById('resPanel-summary');
    if(el)el.innerHTML='<div style="padding:24px;text-align:center;color:'+(isErr?'#FF3B30':'#8E8E93')+'"><div style="font-size:14px;font-weight:600">'+msg+'</div></div>';
  }

  if(dealMode==='portfolio'){fetch('/cpg/api/price',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({eval_date:ev})}).then(function(r){return r.json()}).then(function(d){btn.disabled=false;btn.textContent='▶ Lancer le pricing';if(d.error){alert(d.error);return}pricingDone=true;saveRun({type:'PORTEFEUILLE',notional:d.notional_total},{pv_total:d.pv_total,n:d.count_ok});goStep(3);renderPortfolioResults(d);fetchAndAppendGreeks()}).catch(function(e){btn.disabled=false;btn.textContent='▶ Lancer le pricing';alert(e)});return}

  /* Build deals array */
  var deals=[];
  if(dealMode==='upload'&&extFileDeals&&extFileDeals.length>0){
    deals=extFileDeals;
    console.log('[PRICING] Mode upload, '+deals.length+' deals from file');
  } else {
    ...
    console.log('[PRICING] Mode manual, 1 deal');
  }

  console.log('[PRICING] deals=',JSON.stringify(deals));

  /* Validate minimum fields */
  var d0=deals[0];
  if(!d0.emission||!d0.initial_maturity||!d0.final_maturity){
    btn.disabled=false;btn.textContent='▶ Lancer le pricing';
    alert('Dates manquantes. Vérifiez émission, échéance initiale et finale.');return;
  }
  if(!curveOk){
    btn.disabled=false;btn.textContent='▶ Lancer le pricing';
    alert('Courbe non chargée. Retournez à l\'étape 1.');return;
  }

  /* Navigate to step 3 immediately with loading state */
  goStep(3);
  setStatus('⟳ Pricing en cours... ('+(deals.length===1?'full_results':'price_extendible × '+deals.length)+')');

  var results=[];

  function doPricing(){
    if(deals.length===1){
      var payload=Object.assign({},deals[0],{eval_date:ev});
      console.log('[PRICING] Calling full_results with:',JSON.stringify(payload));
      return fetch('/cpg/api/full_results',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
        .then(function(r){
          console.log('[PRICING] full_results HTTP '+r.status);
          if(!r.ok)throw new Error('HTTP '+r.status);
          return r.json();
        })
        .then(function(d){
          console.log('[PRICING] full_results response:',d.error?'ERROR':'OK, deal_summary='+(!!d.deal_summary));
          if(d.error){setStatus('Erreur : '+d.error.split('\n')[0],true);return}
          results=[d];
        });
    } else {
      ...
      deals.forEach(function(deal,i){
        chain=chain.then(function(){
          return fetch('/cpg/api/price_extendible',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.assign({},deal,{eval_date:ev}))})
            .then(function(r){return r.json()})
            .then(function(d){
              console.log('[PRICING] deal '+i+':',d.error?'ERROR':'OK PV='+d.PV_total);
  ...
  doPricing()
    .then(function(){
      btn.disabled=false;btn.textContent='▶ Lancer le pricing';
      console.log('[PRICING] Done, results='+results.length);
      if(!results.length){setStatus('Le pricing a échoué. Vérifiez la courbe et les paramètres.',true);return}
      pricingDone=true;
      results.forEach(function(r,i){saveRun(deals[i]||{},r)});
      try{
        if(results.length===1&&results[0].deal_summary){
          console.log('[PRICING] Rendering full results');
          renderFullResults(results[0]);
        } else {
          console.log('[PRICING] Rendering portfolio ('+results.length+' results)');
          renderExtPortfolio(results);
        }
        showRes('summary');
        console.log('[PRICING] Render complete');
      }catch(err){
    ...
    .catch(function(e){
      btn.disabled=false;btn.textContent='▶ Lancer le pricing';
      console.error('[PRICING] Fatal:',e);
      setStatus('Erreur fatale : '+e.message,true);
    });
}
...
/* ═══ RENDER: PORTEFEUILLE ═══ */
function renderExtPortfolio(results){var pvT=0,pvF=0,optT=0;results.forEach(function(r){pvT+=r.PV_total||0;pvF+=r.PV_fixed||0;optT+=r.option_value||0});var h='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:18px">';h+=mkKpi('VA Totale',fmt(pvT),'$',results.length+' instruments','var(--green)');h+=mkKpi('VA Fixe',fmt(pvF),'$','','#007AFF');h+=mkKpi('Option',fmt(optT),'$','','#FF9500');h+=mkKpi('Option/VA',fmt(pvT>0?optT/pvT*100:0,1)+' %','','','#5856D6');h+='</div>';h+='<div class="panel block"><div class="panel-header"><div class="dot"></div>Détail</div><div class="panel-body" style="padding:0;overflow-x:auto">';h+=tbl([{t:'Type'},{t:'FundServ'},{t:'Taux',r:1},{t:'VA',r:1,mono:1,bold:1,color:'var(--green)'},{t:'Option',r:1,mono:1,color:'#FF9500'},{t:''}],results.map(function(r){return['<span class="bdg '+(r.cpg_type==='COUPON'?'bdg-g':'bdg-b')+'" style="font-size:9px">'+(r.cpg_type||'?')+'</span>',r.FundServ||r.fundserv||'—',sf(r.client_rate_pct,2)+'%',fmt(r.PV_total),fmt(r.option_value),'<span class="bdg bdg-g" style="font-size:9px">OK</span>']}));h+='</div></div>';document.getElementById('resPanel-summary').innerHTML=h}
